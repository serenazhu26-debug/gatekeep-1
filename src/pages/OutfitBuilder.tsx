  ...
  return (
    <main style={{ width: '100%', minHeight: '100vh', display: 'flex', flexDirection: 'column', padding: '0 24px 40px', background: '#FAF9F6' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '28px 0 0', maxWidth: 1200, margin: '0 auto', width: '100%' }}>
        ...
        <div style={{ maxWidth: 300, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: 13, color: '#94A3B8', fontWeight: 500 }}>
          {shortPrompt}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', width: '100%', flex: 1, display: 'flex', flexDirection: 'column', marginTop: 32 }}>
        ...
        <div style={{ marginBottom: 24 }}>
          <h2 style={{ fontSize: 32, fontWeight: 900, letterSpacing: '-0.04em', color: '#1E293B' }}>Curated for you</h2>
          <p style={{ fontSize: 16, color: '#64748B', marginTop: 4 }}>Swipe to swap items. Everything here is in stock nearby.</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: 40, alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {CATEGORY_ORDER.map(cat => {
              ...
              return (
                <div key={cat}>
                  <p style={{ fontSize: 12, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700, margin: '0 0 8px 4px' }}>
                    {CATEGORY_LABELS[cat]}
                  </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ padding: 24, borderRadius: 24, background: 'white', border: '1px solid #E2E8F0', boxShadow: '0 4px 12px rgba(0,0,0,0.02)' }}>
              <button onClick={() => setShowSlider(v => !v)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'start' }}>
                  <span style={{ fontSize: 13, color: '#94A3B8', fontWeight: 600 }}>Total Cost</span>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 4 }}>
                    <span style={{ fontWeight: 900, fontSize: 36, color: over ? '#EF4444' : '#1E293B', letterSpacing: '-0.04em' }}>${total}</span>
                    <span style={{ fontSize: 14, color: '#94A3B8' }}>/ ${budget} budget</span>
                  </div>
                </div>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: '#F8FAFC', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  {showSlider ? <ChevronUp size={18} color="#64748B" /> : <ChevronDown size={18} color="#64748B" />}
                </div>
              </button>

              {/* Progress bar */}
              <div style={{ marginTop: 20, height: 8, borderRadius: 4, background: '#F1F5F9', overflow: 'hidden' }}>
                <motion.div animate={{ width: `${pct}%` }} transition={{ duration: 0.4 }}
                  style={{ height: '100%', borderRadius: 4, background: over ? '#EF4444' : '#3B82F6' }} />
              </div>
              ...
              <AnimatePresence>
                {showSlider && (
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ marginTop: 24, overflow: 'hidden' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: '#94A3B8', marginBottom: 12, fontWeight: 500 }}>
                      <span>$50</span>
                      <span style={{ color: '#3B82F6', fontWeight: 800 }}>${budget}</span>
                      <span>$500</span>
                    </div>
            ...
            <button onClick={() => navigate('/shopping-list')} disabled={over}
              style={{ width: '100%', padding: '20px', borderRadius: 20, fontWeight: 700, fontSize: 18, color: 'white', background: over ? '#E2E8F0' : '#3B82F6', border: 'none', cursor: over ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, letterSpacing: '-0.01em', boxShadow: over ? 'none' : '0 10px 25px -5px rgba(59, 130, 246, 0.3)', transition: 'all 0.2s' }}>
